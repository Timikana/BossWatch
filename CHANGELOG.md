...
- Nouvelle commande debug `/bossw acdebug` — état de l'engine par frame (supporté, container actif, nombre de boutons).
...
- **Audit combat-lockdown complet suite au bug v0.8.5 — 4 chemins à risque corrigés.** Le déclencheur commun : `/reload` **pendant** un combat (`PLAYER_LOGIN` fire alors avec `InCombatLockdown()` actif) :
  ...
  - `HideBlizzardBossFrames` repositionnait les frames boss Blizzard (protégées) en combat, et son verrou interne empêchait toute nouvelle tentative — les frames Blizzard restaient posées à l'écran toute la session. Désormais : alpha 0 immédiat (autorisé), repositionnement complet différé au sortir de combat.
  - Le **mover** (poignée verte) pouvait démarrer un drag en combat → `ADDON_ACTION_BLOCKED`. Les drags sont refusés en combat (message chat), et l'overlay se rétracte automatiquement à l'entrée en combat puis revient à la sortie.
  - **SoD** : une nameplate disparue en combat laissait la frame pointer sur un token recyclable — si le client réattribuait `nameplateN` à un autre mob, la frame affichait/ciblait le mauvais ennemi jusqu'à la fin du combat. Le slot notifie maintenant immédiatement les consommateurs (nettoyage différé au sortir de combat, garde-fous nil ajoutés).

## [0.8.6] - 2026-07-12

### Corrigé
- **Erreur `ADDON_ACTION_BLOCKED` (`SecureStateDriverManager:SetAttribute`) en combat sur MoP Classic** (rapportée après la v0.8.5). La feature "Cacher sur les combats à un seul boss" ré-enregistrait le state driver de visibilité dans `RefreshAll` en affirmant que `RegisterStateDriver` était autorisé en combat — c'est faux : il écrit un attribut secure sur le `SecureStateDriverManager`, bloqué en combat. Le ré-enregistrement est maintenant gated `InCombatLockdown()` + mis en file pour `PLAYER_REGEN_ENABLED`, et n'écrit que si le driver a réellement changé (cache par frame). Même correctif appliqué au handler de réassignation de slot SoD, qui avait le même bug latent.
...
### Retiré
- **Module `Frames/MyAuras.lua` (tracking combat-log) supprimé.** Introduit en v0.8.0 pour rendre le filtre Auras "Only mine" fiable sur Retail Midnight 12.0+, il s'est avéré inutilisable sur ce client : `RegisterEvent("COMBAT_LOG_EVENT_UNFILTERED")` est désormais rejeté par Blizzard (`ADDON_ACTION_FORBIDDEN`) dès qu'une frame tainted est sur la stack du caller, ce qui arrive en permanence dès qu'un autre addon (ElvUI/DBM/Auctionator…) est actif. Les workarounds successifs (`C_Timer.After`, backoff exponentiel, vérif `IsEventRegistered`) ont tous échoué.
- Conséquence : le filtre **Auras → Source → "Only mine"** revient à son comportement v0.7.x — il fonctionne parfaitement sur **Classic / SoD / TBC Anniversary**, et il est **partiellement fiable sur Retail Midnight** (limitation Blizzard sur `isFromPlayerOrPlayerPet` secret-tagged sur unités hostiles, aucun addon de boss frame ne contourne ça actuellement).

### Corrigé
- **Spam `ADDON_ACTION_FORBIDDEN` toujours présent en v0.8.2** (Klav signale 6+ occurrences sur 12.0.7). Cause = `Frames/MyAuras.lua`, retiré ci-dessus. (rapporté par Klav / warcraftiiitft 12.0.7). Le différer après `PLAYER_LOGIN` ne suffit pas — `BossWatch.lua` a aussi son propre handler `PLAYER_LOGIN` qui crée des SecureUnitButton et taint la thread Lua, donc le `RegisterEvent("COMBAT_LOG_EVENT_UNFILTERED")` de `MyAuras.lua` se retrouve quand même dans une exécution marquée protected. Fix robuste : échapper à la thread courante via `C_Timer.After(0, ...)` qui re-schedule l'enregistrement sur le prochain tick dans un contexte propre.
